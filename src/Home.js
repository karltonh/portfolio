    ...
    const renderHeader = (text, link, body, image) => {
      return  <Grid item class='app' xs= {2} style={{marginTop: "7%", marginLeft: "5%", width: "20%"}}>
                <Link to={link} style={{textDecoration: "none"}}>
                  <Paper variant="contained" square={false} style={{height:"120%", width: "80%", bg:"t", backgroundColor: "#fff6e6"}} onMouseOver={MouseOverPanel} onMouseOut={MouseOffPanel}>
                    <Item style={{backgroundColor: "#fff6e6"}} ><li class="griditem" onMouseOver={MouseOverLink}>{text}</li></Item>
                    ...
                    <ButtonBase sx={{ width: 128, height: 128 }}>{body}</ButtonBase>
                  </Paper>
                </Link>
              </Grid>
    }
